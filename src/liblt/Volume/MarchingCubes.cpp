...
/* Because this piece of code is horrible. */
#ifdef __GNUC__
...
//_____________________________________________________________________________
// Constructor
MarchingCubes::MarchingCubes(const int size_x,
                             const int size_y,
                             const int size_z) :
  _originalMC(false),
  _ext_data  (false),
...
void MarchingCubes::run( real iso ) {
  ...
  for( _i = 0 ; _i < _size_x-1 ; _i++ ) {
    ...
    for( int p = 0 ; p < 8 ; ++p ) {
      ...
      if( Abs( _cube[p] ) < FLT_EPSILON ) _cube[p] = FLT_EPSILON ;
...
void MarchingCubes::compute_intersection_points( real iso ) {
  ...
  for( _i = 0 ; _i < _size_x ; _i++ )
  {
    ...
    if( Abs( _cube[0] ) < FLT_EPSILON ) _cube[0] = FLT_EPSILON ;
    if( Abs( _cube[1] ) < FLT_EPSILON ) _cube[1] = FLT_EPSILON ;
    if( Abs( _cube[3] ) < FLT_EPSILON ) _cube[3] = FLT_EPSILON ;
    if( Abs( _cube[4] ) < FLT_EPSILON ) _cube[4] = FLT_EPSILON ;

    if( _cube[0] < 0 )
    {
      if( _cube[1] > 0 ) set_x_vert( add_x_MCVertex( ), _i,_j,_k ) ;
      if( _cube[3] > 0 ) set_y_vert( add_y_MCVertex( ), _i,_j,_k ) ;
      if( _cube[4] > 0 ) set_z_vert( add_z_MCVertex( ), _i,_j,_k ) ;
    }
    else
    {
      if( _cube[1] < 0 ) set_x_vert( add_x_MCVertex( ), _i,_j,_k ) ;
...
//_____________________________________________________________________________
// Adding vertices
void MarchingCubes::test_MCVertex_addition() {
  if( _nverts >= _Nverts )
  {
...
int MarchingCubes::add_x_MCVertex() {
  test_MCVertex_addition() ;
  MCVertex *vert = _vertices + _nverts++ ;

#ifdef _SMOOTH_VOXELS_
...
int MarchingCubes::add_y_MCVertex() {
  test_MCVertex_addition() ;
  MCVertex *vert = _vertices + _nverts++ ;
...
int MarchingCubes::add_z_MCVertex( ) {
  test_MCVertex_addition() ;
  MCVertex *vert = _vertices + _nverts++ ;
...
int MarchingCubes::add_c_MCVertex( ) {
  test_MCVertex_addition() ;
  MCVertex *vert = _vertices + _nverts++ ;
  ...
  if( vid != -1 ) { ++u ; const MCVertex &v = _vertices[vid] ; vert->x += v.x ;  vert->y += v.y ;  vert->z += v.z ;  vert->nx += v.nx ; vert->ny += v.ny ; vert->nz += v.nz ; }
